...
class Args:
    ...
    wandb_project_name: str = "rnd_hill"
...
def gae(rewards, values, dones, next_value, next_done, gamma, gae_lambda, device):
...
class RND(nn.Module):
    def __init__(self, envs, embed_dim):
        super().__init__()
        self.predictor = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64, embed_dim)),
        )   
        self.target = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), 64), std=np.sqrt(2) * 1.5),
            nn.Tanh(),
            layer_init(nn.Linear(64, embed_dim), std=np.sqrt(2) * 1.5),
        )
    ...
    def get_intr_reward(self, x):
        return ((self.get_target(x) - self.get_prediction(x)) ** 2).mean(axis=-1)
...
if __name__ == "__main__":
    ...
    # extra observation for calculating intrinsic reward at last step
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            start_step = torch.zeros(args.num_envs, dtype=torch.int32)
                ...
                if "final_info" in infos:
                    for i, info in enumerate(infos["final_info"]):
                        if info and "episode" in info:
                            ...
                            episodic_return_intr = rewards_intr[start_step[i]:step+1, i].sum().item()
                            writer.add_scalar("charts/episodic_return_intr", episodic_return_intr, global_step)
                            start_step[i] = step + 1
        obs[-1] = next_obs
